...
import { History, X, Sparkles, ChevronRight, Clock, Trash2 } from "lucide-react";
import type { BusinessAnalysisResult } from "@/types/business-analysis";

interface HistoryDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectAnalysis: (analysis: BusinessAnalysisResult) => void;
}
...
export function HistoryDrawer({
  ...
  const [historyList, setHistoryList] = useState<BusinessAnalysisResult[]>([]);
  ...
  const handleClearHistory = () => {
    localStorage.removeItem("idea_analyzer_history");
    setHistoryList([]);
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-bold text-white font-heading">
                Riwayat Analisis Ide
              </h3>
              <p className="text-xs text-slate-400">Daftar ide bisnis yang pernah dianalisis</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-3">
          {historyList.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400 space-y-3">
              ...
              <p className="text-xs">Belum ada riwayat analisis ide yang tersimpan.</p>
            </div>
          ) : (
            historyList.map((item, idx) => (
              <div
                ...
                className="group cursor-pointer rounded-2xl border border-white/10 bg-slate-950/60 p-4 transition-all duration-200 hover:border-indigo-500/50 hover:bg-slate-950 hover:shadow-lg"
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="text-sm font-bold text-white group-hover:text-indigo-300 transition-colors">
                      {item.input?.ideaName || "Ide Bisnis"}
                    </h4>
                    <p className="text-xs text-slate-400 mt-0.5 line-clamp-1">
                      {item.meta?.tagline || item.input?.targetMarket}
                    </p>
                  </div>
