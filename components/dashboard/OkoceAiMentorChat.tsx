...
How can I assist you with your 14-day action plan or operational roadmap today? Feel free to pick a quick question below!`,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn no-print">
      <div className="relative flex h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-indigo-500/30 bg-slate-900 shadow-2xl">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-white/10 bg-indigo-950/40 px-6 py-4">
        ...
        <div className="flex items-center justify-between border-b border-white/10 bg-slate-950/60 px-6 py-2 text-xs">
          ...
          <span className="hidden sm:block text-[11px] text-indigo-300 font-semibold">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {isTyping && (
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-xs text-indigo-300">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {QUICK_PROMPTS.map((promptText, idx) => (
              <button
                ...
                className="flex items-center gap-1 rounded-xl border border-white/10 bg-slate-900/80 px-3 py-1.5 text-[11px] text-slate-300 hover:border-indigo-500/50 hover:bg-slate-800 hover:text-white whitespace-nowrap transition-all"
              >
        ...
        >
          <input
            ...
            disabled={isTyping}
            className="flex-1 rounded-xl border border-white/10 bg-slate-900 px-4 py-3 text-xs text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
          />
          <button
            type="submit"
            disabled={!inputMessage.trim() || isTyping}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50 transition-colors shadow-md shadow-indigo-500/20"
          >
