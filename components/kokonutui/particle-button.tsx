    ...
    <div className="relative inline-block overflow-visible">
      <button
        ...
        className={cn(
          ...
          variant === "primary" &&
            "bg-gradient-to-r from-red-600 via-red-500 to-red-600 px-6 py-3 text-sm text-white shadow-lg shadow-red-600/30 hover:shadow-red-500/50 hover:brightness-110",
          variant === "glow" &&
            "border border-red-500/40 bg-slate-900/80 px-4 py-2 text-xs text-red-300 backdrop-blur-md hover:border-red-400 hover:bg-red-950/60 hover:text-white",
          className
        )}
        {...props}
      >
        ...
        {children ? (
          ...
          <>
            <Sparkles className="h-4 w-4 text-red-200 group-hover:rotate-12 transition-transform" />
      ...
      {particles.map((p) => (
        <motion.span
          ...
          className="pointer-events-none absolute h-1.5 w-1.5 rounded-full bg-red-400 shadow-sm shadow-red-300"
          style={{ left: 0, top: 0 }}
        />
      ))}
    </div>
